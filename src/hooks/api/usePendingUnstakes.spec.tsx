...
import { usePendingUnstakes } from './usePendingUnstakes';
...
describe('usePendingUnstakes', () => {
  ...
  test('should not fetch if address is empty', async () => {
    const { result } = renderHook(() => usePendingUnstakes(''), { wrapper });
  ...
  test('should fetch and transform data correctly', async () => {
    ...
    const { result } = renderHook(() => usePendingUnstakes(address), { wrapper });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(mockedAxios.get).toHaveBeenCalledWith('/api/unstake/pending', {
      params: { address },
    });
