...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const canisterResponse = await canister.stake(unsignedPsbt);
    if (canisterResponse.error) {
      logger.error('Canister stake failed', { error: canisterResponse.error });
      ...
      try {
        ...
        logger.debug(`Unlocked ${utxos.length} UTXOs after canister stake failure`);
      } catch (unlockError) {
    ...
    const existingStake = await db.stake.getByTxid(tempTxid);
    if (existingStake) {
      logger.debug('Stake transaction already exists', { tempTxid });
