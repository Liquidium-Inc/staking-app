...
const mock = vi.hoisted(() => ({
  ...
  db: {
    stake: { getAfterBlock: vi.fn(), update: vi.fn() },
  },
}));
...
describe('GET', () => {
  ...
  it('returns success and updates stakes as expected', async () => {
    ...
    mock.mempool.transactions.getTxStatus
      ...
      .mockResolvedValueOnce({ confirmed: true, block_height: 90 });
    mock.db.stake.getAfterBlock.mockResolvedValue([
      { id: 1, txid: 'tx1' },
      { id: 2, txid: 'tx2' },
    ]);
    ...
    expect(mock.db.stake.getAfterBlock).toHaveBeenCalledWith(100 - expectedConfirmations);
    expect(mock.mempool.transactions.getTxStatus).toHaveBeenCalledTimes(2);
    expect(mock.db.stake.update).toHaveBeenCalledWith([1, 2], { block: 90 });
  });

  it('does not update if confirmations are not enough', async () => {
    mock.mempool.blocks.getBlocksTipHeight.mockResolvedValue(95);
    mock.db.stake.getAfterBlock.mockResolvedValue([{ id: 1, txid: 'tx1' }]);
    ...
    expect(mock.db.stake.update).not.toHaveBeenCalled();
    const json = await res.json();
    expect(json).toEqual({ success: true });
  });
  ...
  it('filters out unconfirmed transactions', async () => {
    ...
    expect(mock.db.stake.update).toHaveBeenCalledWith([2], { block: 90 });
    const json = await res.json();
    expect(json).toEqual({ success: true });
  });
});
